...
def get_slime_extra_args_provider(add_custom_arguments=None):
    def add_slime_arguments(parser):
        def add_zce_arguments(parser):
            parser.add_argument(
                "--zce-rate-add-position",
                type=str,
                choices=["reward", "loss", "advantage", "none", "showzce"],
                default="none",
        ...
        parser = add_zce_arguments(parser)
...
def slime_validate_args(args):
    ...
    if args.rollout_max_response_len is not None and args.rollout_max_prompt_len is not None:
        assert (
            args.rollout_max_prompt_len <= args.rollout_max_context_len - 1
