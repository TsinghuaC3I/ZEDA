...
class RolloutManager:
    ...
    def generate(self, rollout_id):
        ...
        try:
            ...
            data = self._convert_samples_to_train_data(data, rollout_id=rollout_id)
    ...
    def _post_process_rewards(self, samples: list[Sample] | list[list[Sample]], rollout_id):
        if self.custom_reward_post_process_func is not None:
            log_dict = {}
            my_experts_info = []
            if self.args.use_response_info_only:
                my_experts_info = [torch.tensor(sample.rollout_routed_experts[(-1*sample.response_length+1):]) for sample in samples]
            else:
                my_experts_info = [torch.tensor(sample.rollout_routed_experts) for sample in samples]
            
            binary_tensors = [(tensor > (self.args.num_experts - 1)).float() for tensor in my_experts_info]
            zce_rates = [tensor.mean().item() for tensor in binary_tensors]

            log_dict["rollout/mean_seq_zce_r"] = np.mean(zce_rates)
            log_dict["rollout/std_seq_zce_r"] = np.std(zce_rates)

            del binary_tensors, zce_rates, my_experts_info

            logger.info(f"ZCE {rollout_id}: {log_dict}")
        ...
        log_dict["rollout/original_raw_reward"] = np.mean(raw_rewards)
        
        if self.args.zce_rate_add_position == "reward" and self.args.rm_type != "dapo":
            my_experts_info = []
            if self.args.use_response_info_only:
                my_experts_info = [torch.tensor(sample.rollout_routed_experts[(-1*sample.response_length+1):]) for sample in samples]
            else:
            ...
            raw_rewards = [reward*(1+self.args.zce_coef*zce_rate) for reward, zce_rate in zip(raw_rewards, zce_rates)]
            del binary_tensors, zce_rates, my_experts_info
        elif self.args.zce_rate_add_position == "reward" and self.args.rm_type == "dapo":
            my_experts_info = []
            if self.args.use_response_info_only:
                my_experts_info = [torch.tensor(sample.rollout_routed_experts[(-1*sample.response_length+1):]) for sample in samples]
            else:
            ...
            raw_rewards = [reward*(1+self.args.zce_coef*zce_rate) for reward, zce_rate in zip(raw_rewards, zce_rates)]
            del binary_tensors, zce_rates, my_experts_info
        elif self.args.zce_rate_add_position == "none":
            pass
        else:
            my_experts_info = []
            if self.args.use_response_info_only:
        ...
        if (
            ...
            if self.args.advantage_estimator in ["grpo", "gspo"] and self.args.grpo_std_normalization and self.args.use_std_norm == "yes":
    ...
    def _convert_samples_to_train_data(self, samples: list[Sample] | list[list[Sample]], rollout_id):
        """
        Convert inference generated samples to training data.
        """
        raw_rewards, rewards = self._post_process_rewards(samples, rollout_id=rollout_id)
...
def _compute_zero_std_metrics(args, all_samples: list[Sample]):
    ...
    if args.advantage_estimator in ["ppo", "on_policy_distillation"]:
        return {}
