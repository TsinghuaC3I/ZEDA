...
def train(
    ...
    if args.use_moe_router_tracker:
        ...
        if _moe_router_tracker is None:
            ...
            _moe_router_tracker.register_hooks(model)
    ...
    for step_id in range(num_steps_per_rollout):
        ...
        if args.num_experts is not None:
            ...
            logger.info(f"Performing moe metrics tracking for step {accumulated_step_id}")
            moe_stats = {}
            ...
            if "grouped_aux_loss" in args.moe_router_load_balancing_type:
                track_names.append("grouped_load_balancing_loss")
