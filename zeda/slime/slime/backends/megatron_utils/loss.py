...
def compute_advantages_and_returns(args: Namespace, rollout_data: RolloutBatch) -> None:
    ...
    my_experts_info: list[torch.Tensor] = rollout_data.get("rollout_routed_experts")
    ...
    if args.advantage_estimator in ["grpo", "gspo"]:
        ...
        if args.zce_rate_add_position == "advantage":
            if args.use_response_info_only:
                my_experts_info = [my_experts_info[i][(-response_lengths[i]+1):] for i in range(len(my_experts_info))]
            binary_tensors = [(tensor > (args.num_experts - 1)).float() for tensor in my_experts_info]
            zce_rates = torch.tensor([tensor.mean().item() for tensor in binary_tensors], dtype=torch.float32, device=kl[0].device)
            # rewards = rewards * (1 + args.zce_coef * zce_rates)  # zce_loss_coef, 越大会越鼓励调用zce
            rewards = rewards + args.zce_coef * zce_rates 
