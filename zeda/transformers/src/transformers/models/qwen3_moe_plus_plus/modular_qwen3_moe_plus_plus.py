...
from .configuration_qwen3_moe_plus_plus import Qwen3MoePlusPlusConfig

logger = logging.get_logger(__name__)


class Qwen3MoeAttention(Qwen3Attention):  # This is the main diff with qwen2Moe!
    def __init__(self, config: Qwen3MoePlusPlusConfig, layer_idx: int):
...
class Qwen3MoePlusPlusSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        assert self.total_num_experts == len(self.experts), "The total number of experts should be equal to the sum of the number of experts and the number of ZCE experts"
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_zce_mask:
            ...
            zce_mask[:, self.num_experts:] = 1
            router_logits = router_logits.masked_fill(zce_mask, -np.inf)
...
class Qwen3MoePlusPlusDecoderLayer(Qwen2MoeDecoderLayer):
    def __init__(self, config: Qwen3MoePlusPlusConfig, layer_idx: int):
        ...
        if (layer_idx not in config.mlp_only_layers) and (
            ...
            self.mlp = Qwen3MoePlusPlusSparseMoeBlock(config)
        else:
...
class Qwen3MoePlusPlusModel(MixtralModel):
    pass


class Qwen3MoePlusPlusForCausalLM(MixtralForCausalLM):
    def __init__(self, config):
        super().__init__(config)
        self.model = Qwen3MoePlusPlusModel(config)
        self.num_experts = config.num_experts
        self.total_num_experts = config.num_experts + sum(config.zce_nums)
...
__all__ = [
    ...
    "Qwen3MoePlusPlusModel",
    "Qwen3MoePlusPlusPreTrainedModel", 
]
