...
class TransformerConfig(ModelParallelConfig):
    ...
    - "grouped_aux_loss": Group-level aux loss that separates normal experts (E) and
    zero-compute experts (Z) into two groups. This encourages routing to zero experts
    without enforcing balancing among normal experts individually. Requires num_zero_experts > 0.
    Formula: L_aux = \\alpha * (E+Z)/K * (1/E * f_E * P_E + 1/Z * f_Z * P_Z)
    ...
    moe_zero_expert_weight: float = 1.0
    """Coefficient to adjust the expected proportion of zero experts in grouped_aux_loss.
    When set to w, the expected proportion of zero experts becomes:
    w * num_zero_experts / (w * num_zero_experts + num_normal_experts)
    Default is 1.0, which gives equal weight per expert:
    num_zero_experts / (num_zero_experts + num_normal_experts)
    Higher values encourage more traffic to zero experts, lower values encourage less.
    """
    ...
    def __post_init__(self):
        ...
        # Validate grouped_aux_loss requires num_zero_experts > 0
        load_balancing_types = (
            self.moe_router_load_balancing_type
            if isinstance(self.moe_router_load_balancing_type, list)
            else [self.moe_router_load_balancing_type]
        )
        if "grouped_aux_loss" in load_balancing_types:
            if self.num_zero_experts is None or self.num_zero_experts <= 0:
                raise ValueError(
                    "moe_router_load_balancing_type='grouped_aux_loss' requires num_zero_experts > 0. "
                    "The grouped aux loss is designed to encourage routing to zero-compute experts."
                )
