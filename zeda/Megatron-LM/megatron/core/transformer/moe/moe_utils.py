...
def grouped_load_balancing_loss_func(
    probs: torch.Tensor,
    tokens_per_expert: torch.Tensor,
    total_num_tokens: int,
    topk: int,
    num_experts: int,
    num_normal_experts: int,
    moe_aux_loss_coeff: float,
    zero_expert_weight: float = 1.0,
):
    """Calculate the grouped auxiliary loss for load balancing.

    This function computes a group-level aux loss that separates normal experts (E)
    and zero-compute experts (Z) into two groups. This encourages routing to zero
    experts without enforcing balancing among normal experts individually.

    ### Detailed explanation of the grouped auxiliary loss #######

    The formula for the grouped auxiliary loss is:
        loss = \\alpha * (E+w*Z)/K * (1/E * f_E * P_E + 1/(w*Z) * f_Z * P_Z)

    where:
        - E: number of normal experts (num_normal_experts)
        - Z: number of zero-compute experts (num_experts - num_normal_experts)
        - K: topk (number of experts selected per token)
        - \\alpha: moe_aux_loss_coeff
        - w: zero_expert_weight (coefficient to adjust expected zero expert proportion)
        - f_E = Σ_{i=1}^{E} f_i (total token fraction routed to normal experts)
        - P_E = Σ_{i=1}^{E} P_i (total router probability for normal experts)
        - f_Z = Σ_{i=E+1}^{E+Z} f_i (total token fraction routed to zero experts)
        - P_Z = Σ_{i=E+1}^{E+Z} P_i (total router probability for zero experts)
        - f_i = tokens_per_expert[i] / (total_num_tokens * topk)
        - P_i = sum(probs[:, i]) / total_num_tokens

    When zero_expert_weight = w, the expected proportion of zero experts is:
        w * Z / (w * Z + E)
    When w = 1.0 (default), this reduces to Z / (Z + E).
    ...
    Args:
        ...
        total_num_tokens (int): Total number of tokens in the batch.
        topk (int): The number of experts selected for each token.
        num_experts (int): Total number of experts (E + Z).
        num_normal_experts (int): Number of normal experts (E).
        moe_aux_loss_coeff (float): The coefficient for the auxiliary loss.
        zero_expert_weight (float): Coefficient to adjust expected zero expert proportion.
                                    Default is 1.0. Higher values encourage more routing to
                                    zero experts, lower values encourage less.

    Returns:
        torch.Tensor: The grouped auxiliary loss for load balancing.
    """
    num_zero_experts = num_experts - num_normal_experts

    # Safety check - this should be caught earlier in config validation
    if num_zero_experts <= 0:
        raise ValueError(
            f"grouped_load_balancing_loss_func requires num_zero_experts > 0, "
            f"got num_experts={num_experts}, num_normal_experts={num_normal_experts}"
        )

    # Compute aggregated probabilities per expert: P_i * T (sum over tokens)
    # Shape: [num_experts]
    aggregated_probs_per_expert = probs.sum(dim=0)

    # Split into normal expert group and zero expert group
    # Normal experts: indices [0, num_normal_experts)
    # Zero experts: indices [num_normal_experts, num_experts)
    tokens_normal = tokens_per_expert[:num_normal_experts]  # Shape: [E]
    tokens_zero = tokens_per_expert[num_normal_experts:]    # Shape: [Z]
    probs_normal = aggregated_probs_per_expert[:num_normal_experts]  # Shape: [E]
    probs_zero = aggregated_probs_per_expert[num_normal_experts:]    # Shape: [Z]

    # Compute group-level aggregates:
    # f_E = sum(f_i for i in normal experts) = sum(tokens_normal) / (T * K)
    # P_E = sum(P_i for i in normal experts) = sum(probs_normal) / T
    # Similarly for zero experts
    f_E = tokens_normal.sum()  # Will normalize later
    f_Z = tokens_zero.sum()    # Will normalize later
    P_E = probs_normal.sum()   # Will normalize later
    P_Z = probs_zero.sum()     # Will normalize later

    # The normalization factor for the full formula with zero_expert_weight (w):
    # loss = \\alpha * (E+w*Z)/K * (1/E * f_E * P_E + 1/(w*Z) * f_Z * P_Z)
    #
    # Where f_E = sum(tokens_normal) / (T * K), P_E = sum(probs_normal) / T
    # So f_E * P_E = sum(tokens_normal) * sum(probs_normal) / (T^2 * K)
    #
    # Full loss = \\alpha * (E+w*Z)/K * (1/E * f_E * P_E + 1/(w*Z) * f_Z * P_Z)
    #           = \\alpha * (E+w*Z)/K * (1/E * sum(tokens_normal) * sum(probs_normal) / (T^2 * K)
    #                         + 1/(w*Z) * sum(tokens_zero) * sum(probs_zero) / (T^2 * K))
    #           = \\alpha * (E+w*Z) / (K^2 * T^2) * (f_E * P_E / E + f_Z * P_Z / (w*Z))
    #
    # This ensures the expected proportion of zero experts is:
    #   w * Z / (w * Z + E)

    # Compute scaled contributions for each group
    # Avoid divide by zero (E should always be > 0 for MoE to make sense)
    normal_term = (f_E * P_E) / num_normal_experts if num_normal_experts > 0 else 0.0
    # Apply zero_expert_weight to the denominator: divide by (Z * w) instead of Z
    zero_term = (f_Z * P_Z) / (num_zero_experts * zero_expert_weight)
    
    weighted_num_experts = num_normal_experts + zero_expert_weight * num_zero_experts

    aux_loss = (normal_term + zero_term) * (
        weighted_num_experts * moe_aux_loss_coeff / (topk * total_num_tokens * total_num_tokens)
    )
