...
from megatron.core.transformer.moe.moe_utils import (
    ...
    grouped_load_balancing_loss_func,
...
class TopKRouter(Router):
    ...
    def is_aux_loss_enabled(self) -> bool:
        ...
        for aux_loss_type in ["aux_loss", "seq_aux_loss", "grouped_aux_loss"]:
    ...
    def _apply_grouped_aux_loss(
        self, probs: torch.Tensor, scores_for_aux_loss: torch.Tensor, routing_map: torch.Tensor
    ):
        """Apply the grouped auxiliary loss for the given scores and routing map.

        This loss separates normal experts (E) and zero-compute experts (Z) into two groups,
        encouraging routing to zero experts without enforcing balancing among normal experts.

        Formula: L_aux = \\alpha * (E+w*Z)/K * (1/E * f_E * P_E + 1/(w*Z) * f_Z * P_Z)
        where w = moe_zero_expert_weight
        """
        grouped_aux_loss_coeff = self.get_aux_loss_coeff("grouped_aux_loss")
        if grouped_aux_loss_coeff == 0:
            return probs
        ...
        aux_loss = grouped_load_balancing_loss_func(
            probs=scores_for_aux_loss,
            tokens_per_expert=tokens_per_expert,
            total_num_tokens=total_num_tokens,
            topk=self.topk,
            num_experts=self.num_experts,
            num_normal_experts=self.config.num_moe_experts,
            moe_aux_loss_coeff=grouped_aux_loss_coeff,
            zero_expert_weight=self.config.moe_zero_expert_weight,
        )
        probs = self.attach_and_log_load_balancing_loss(
            probs, grouped_aux_loss_coeff, aux_loss, "grouped_load_balancing_loss", self.tp_cp_group
        )
        return probs
    ...
    def routing(self, logits: torch.Tensor):
        ...
        if self.training and torch.is_grad_enabled() and self.is_aux_loss_enabled():
            ...
            probs = self._apply_grouped_aux_loss(probs, scores_for_aux_loss, routing_map_for_aux_loss)
