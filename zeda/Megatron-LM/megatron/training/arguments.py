...
def core_transformer_config_from_args(args, config_class=None):
    ...
    if hasattr(args, 'num_zero_experts'):
        ...
        kw_args['zero_experts_type'] = args.zero_experts_type
...
def _add_network_size_args(parser):
    ...
    group.add_argument('--zero-experts-type', type=str, default=None,
                       choices=['zero', 'copy'],
                       help='Type of zero experts.')
...
def _add_moe_args(parser):
    ...
    group.add_argument('--moe-router-load-balancing-type', nargs='+', type=str,
                       choices=['aux_loss', 'seq_aux_loss', 'grouped_aux_loss', 'sinkhorn', 'none'],
                       default='aux_loss',
                       help='Determines the load balancing strategy for the router. "aux_loss" corresponds to the load balancing loss used in GShard and SwitchTransformer; "seq_aux_loss" corresponds to the load balancing loss used in DeepSeekV2, which computes the loss for each individual sample; "grouped_aux_loss" is a group-level aux loss that separates normal experts (E) and zero-compute experts (Z) into two groups, encouraging routing to zero experts without enforcing balancing among normal experts (requires --num-zero-experts > 0); "sinkhorn" corresponds to the balancing algorithm used in S-BASE, and "none" implies no load balancing. The default is "aux_loss". Multiple types can be combined, e.g., "aux_loss grouped_aux_loss".')
    ...
    group.add_argument('--moe-zero-expert-weight', type=float, default=1.0,
                       help='Coefficient to adjust the expected proportion of zero experts in grouped_aux_loss. '
                       'When set to x, the expected proportion of zero experts becomes: '
                       'x * num_zero_experts / (x * num_zero_experts + num_normal_experts). '
                       'Default is 1.0 (equal weight per expert). '
                       'Higher values encourage more traffic to zero experts.')
